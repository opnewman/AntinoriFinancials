...
const RiskStatsJobManager = () => {
  ...
  // Start a turbo risk stats update (completes in 2-3 seconds)
  const startTurboRiskStatsUpdate = async () => {
    try {
      setJobState(prev => ({ ...prev, isLoading: true, error: null }));
      
      const result = await window.api.updateRiskStatsTurbo();
      
      if (result.success) {
        // For turbo updates, we don't have a job ID since they complete immediately
        // Create a synthetic job object with completed status
        const completedJob = {
          success: true,
          status: 'completed',
        ...
        throw new Error(result.error || 'Failed to start turbo update');
      }
    } catch (error) {
      setJobState(prev => ({ 
        ...prev, 
        isLoading: false, 
        error: error.message || 'Failed to start turbo risk stats update' 
      }));
      
      if (typeof toast === 'function') {
        toast({
          title: 'Error',
          description: error.message || 'Failed to start turbo risk stats update',
          status: 'error',
          duration: 5000,
          isClosable: true,
        });
